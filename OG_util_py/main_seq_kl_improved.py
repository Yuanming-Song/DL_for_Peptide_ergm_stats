...
def main(): 
    ...
    ## print the number of parameters and the model architecture
    print(f"Number of parameters: {sum(p.numel() for p in model.parameters())}")
    print(model)
    
    # In main_seq_kl_improved.py
    # Adjust optimizer settings
    optimizer = optim.AdamW([
        {'params': model.decoder.e1.parameters(), 'lr': args.lr * 1.0},
        {'params': model.decoder.e2.parameters(), 'lr': args.lr * 0.8},
        {'params': model.decoder.e3.parameters(), 'lr': args.lr * 0.6},
        {'params': model.decoder.e4.parameters(), 'lr': args.lr * 0.4},
        {'params': model.decoder.e5.parameters(), 'lr': args.lr * 0.2},
        {'params': model.encoder.parameters(), 'lr': args.lr * 0.5},  # Also include encoder parameters
    ...
    # Create a file to store batch-level metrics
    batch_metrics_file = os.path.join(args.base_dir, 'batch_metrics.csv')
    with open(batch_metrics_file, 'w') as f:
        f.write("batch,epoch,loss,grad_norm,clip_percent\n")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (enc_inputs, labels) in enumerate(train_loader):
            ...
            print(f"Gradient norm without clipping: {grad_norm.item():.6f}")

            # Gradient clipping
            grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=args.grad_clip)
            grad_norm_percentage = (grad_norm > args.grad_clip).sum() / len(list(model.parameters()))
            print(f"(batch: {batch_idx}, epoch: {epoch+1},) Gradient clipping percentage: {grad_norm_percentage:.2%}")
            param_norm = torch.norm(torch.stack([torch.norm(p) for p in model.parameters()]))
            
            # Save batch metrics
            with open(batch_metrics_file, 'a') as f:
                f.write(f"{batch_idx},{epoch+1},{loss.item():.6f},{grad_norm.item():.6f},{grad_norm_percentage:.6f}\n")
        ...
        # Log metrics - ensure all tensors are moved to CPU before creating DataFrame
        log_data = {
            'epoch': [epoch + 1],
            'train_loss': [float(train_loss)],
            'valid_kl': [float(valid_kl)],
            'learning_rate': [float(optimizer.param_groups[0]["lr"])],
            'best_valid_kl': [float(best_valid_kl)],
            'patience_counter': [int(patience_counter)],
            'max_prob_error': [float(max_prob_error)],
            'avg_grad_norm': [float(avg_grad_norm)],
            'avg_param_norm': [float(avg_param_norm)]
        }

        # Save epoch metrics to CSV - append mode
        epoch_metrics_df = pd.DataFrame(log_data)
        epoch_metrics_file = os.path.join(args.base_dir, 'epoch_metrics.csv')
        if epoch == 0:
            # Write header only for the first epoch
            epoch_metrics_df.to_csv(epoch_metrics_file, index=False)
        else:
            # Append without header for subsequent epochs
            epoch_metrics_df.to_csv(epoch_metrics_file, mode='a', header=False, index=False)
